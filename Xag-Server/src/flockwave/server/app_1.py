...
class SkybrushServer(DaemonApp):
    ...
    async def socket_response(
        ...
        if msg == "geofence":
            # coords come from the currently-selected map feature (GCS-live's
            # SwarmPanel "Set Origin" button), in OpenLayers [lon, lat] order,
            # same as every other coordinate-taking message here.
            raw_points = parameters.get("coords") or []
            points = [[float(p[1]), float(p[0])] for p in raw_points]
            if not points:
                result = "geofence needs a selected area with at least one point"
            else:
                result = update_geofence_origin(points)

        if msg == "home":
            # rtl_height is currently unused: "home" has no real effect on
            # the swarm side (see swarm.py::home_socket) -- the real landing
            # path is "landing" (landing_mission_send). This .get() only
            # fixes a crash (was `parameters.pop()` with no key, which always
            # raised TypeError); it does not give "home" new behavior.
            rtl_height = parameters.get("rtl_height", 50)
            result = home_socket()
        ...
        if msg == "specific_bot_goal":
            stop_socket()
            await sleep(2)
            # goal is [lat, lon]; specific_bot_goal_socket's signature changed
            # from (drone_num, goal_num) to (uav, lat, lon) since passing a
            # coordinate pair into a comma-joined string broke the swarm
            # side's parsing (see swarm.py::specific_bot_goal_socket).
            goal = parameters["goal"]
            result = specific_bot_goal_socket(parameters["uav"], goal[0], goal[1])

        if msg == "goal":
            coords = parameters.get("coords")
            ids = parameters.get("ids") or []
            if len(ids) >= 1:
                # A specific UAV selection means a per-bot goal, not a
                # swarm-wide one -- selectedUAVIds was already being sent
                # by the GUI but silently ignored here. The swarm receiver
                # only tracks one "mission in progress" at a time and
                # rejects any specific_bot_goal sent while a previous one
                # is still active ("Send 'stop' first"), so firing all of
                # them back to back drops every command after the first --
                # each dispatch gets its own stop + settle time instead.
                lon, lat = coords[0][0], coords[0][1]
                results = []
                for uav_id in ids:
                    stop_socket()
                    await sleep(1)
                    results.append(specific_bot_goal_socket(uav_id, lat, lon))
                result = all(results)
            else:
                stop_socket()
                await sleep(2)
                ...
                result = goal_socket(coords, direction, parameters.get("radius"))
        ...
        if msg == "loiter":
            ...
            # radius was previously never sent end-to-end (missing here, and
            # the swarm-side prefix/field-count didn't even match what
            # loiter() sent) -- "loiter" was fully non-functional before this
            # fix. Default matches TimeCalculation's existing loiter_radius=200.
            radius = parameters.get("radius", 200)
            result = loiter(center_latlon, direction, radius)
    ...
    async def dispatch_to_uavs(
        ...
        if message_type == "X-UAV-TAKEOFF":
            ...
            # uav = self.find_uav_by_id(uav_ids[-1])
            # print(uav.status.heading)
