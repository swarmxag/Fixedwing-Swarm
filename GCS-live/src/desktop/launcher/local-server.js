...
const search = async (paths) => {
  // Auto-detection (and therefore auto-launching) of a local server
  // executable is a packaged-app-only convenience. When running from
  // source (`npm run start:electron` etc.) the developer is expected to
  // start skybrushd and vtol_demo.exe manually in their own terminals, so
  // we deliberately report "not found" here rather than picking up
  // whatever happens to be on PATH.
  if (!app.isPackaged) {
    localServerPath = undefined;
    if (localServerPathDeferred) {
      localServerPathDeferred.resolve(undefined);
