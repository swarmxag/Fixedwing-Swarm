...
const SwarmPanel = ({
  ...
  const handlePoint = async (message) => {
    ...
    // "Automate Goals" fans a single dropped point into one loiter centre per
    // UAV -- it only makes sense with exactly one point (and more than one UAV
    // to spread along the bearing).
    if (
      message === 'goal' &&
      socketData.automateGoals &&
      (data.points?.length ?? 0) !== 1
    ) {
      dispatch(
        showError('Automate Goals expects exactly one dropped goal point')
      );
      return;
    }
    try {
        ...
        <Box style={{ display: 'flex', gap: 20, alignItems: 'center' }}>
          ...
          >
            <FormControlLabel
              control={
                <Checkbox
                  checked={Boolean(socketData.automateGoals)}
                  onChange={(event) =>
                    dispatch(
                      setAutomateGoals({ automateGoals: event.target.checked })
                    )
                  }
                />
              }
              label='Automate Goals (one point → per-UAV loiter centres)'
            />
            <FormControl variant='standard' style={{ width: 110 }}>
              <InputLabel htmlFor='automateBearing'>Bearing °</InputLabel>
              <Input
                id='automateBearing'
                type='number'
                disabled={!socketData.automateGoals}
                value={socketData.automateBearing ?? 90}
                onChange={(event) =>
                  dispatch(
                    changeAutomateBearing({
                      automateBearing: Number(event.target.value),
                    })
                  )
                }
              />
            </FormControl>
            <FormControl variant='standard' style={{ width: 140 }}>
              <InputLabel htmlFor='automateSafetyMargin'>
                Safety margin (m)
              </InputLabel>
              <Input
                id='automateSafetyMargin'
                type='number'
                disabled={!socketData.automateGoals}
                value={socketData.automateSafetyMargin ?? 50}
                onChange={(event) =>
                  dispatch(
                    changeAutomateSafetyMargin({
                      automateSafetyMargin: Number(event.target.value),
                    })
                  )
                }
              />
