...
class Bot:
    ...
    def step(self, step_size=1.0):  # 0.085 speed 20
    ...
    def move(self, direction, speed, step_size=0.8):
        ...
        speed_used = min(speed, self.max_speed)
        pos_before = (self.x, self.y)
        print(
            f"[BOT DEBUG] speed_requested={speed:.3f} max_speed={self.max_speed:.3f} "
            f"speed_used={speed_used:.3f} step_size={step_size:.3f} "
            f"step_distance={speed_used * step_size:.3f} "
            f"pos_before=({pos_before[0]:.2f},{pos_before[1]:.2f})"
        )

        self.step(speed_used * step_size)

        print(
            f"[BOT DEBUG] pos_after=({self.x:.2f},{self.y:.2f}) theta={self.theta:.3f}"
        )
        """
