...
try:	
	vehicle1= connect('udpin:192.168.6.151:14551',baud=115200, heartbeat_timeout=heartbeat_ip_timeout[0])
...
try:		
	vehicle2= connect('udpin:192.168.6.151:14552',baud=115200,heartbeat_timeout=heartbeat_ip_timeout[1])
...
try:
	vehicle3= connect('udpin:192.168.6.151:14553',baud=115200,heartbeat_timeout=heartbeat_ip_timeout[2])
...
try:		
	vehicle4= connect('udpin:192.168.6.151:14554',baud=115200,heartbeat_timeout=heartbeat_ip_timeout[3])
...
try:		
	vehicle5= connect('udpin:192.168.6.151:14555',baud=115200,heartbeat_timeout=heartbeat_ip_timeout[4])
...
try:
	vehicle6= connect('udpin:192.168.6.151:14556',baud=115200,heartbeat_timeout=heartbeat_ip_timeout[5])
...
try:
	vehicle7= connect('udpin:192.168.6.151:14557',baud=115200,heartbeat_timeout=heartbeat_ip_timeout[6])
...
try:
	vehicle8= connect('udpin:192.168.6.151:14558',baud=115200,heartbeat_timeout=heartbeat_ip_timeout[7])
...
try:	
	vehicle9= connect('udpin:192.168.6.151:14559',baud=115200,heartbeat_timeout=heartbeat_ip_timeout[8])
...
try:	
	vehicle10= connect('udpin:192.168.6.151:14560',baud=115200,heartbeat_timeout=heartbeat_ip_timeout[9])
...
slave_heal_ip=["192.168.6.151"]*len(vehicles)
	...
	try:            
		...
		if(data==b"takeoff"):
			...
			for i, vehicle in enumerate(vehicles):
			    print(i)
			    '''
			    altitude = 30 + i  # Adjust the starting altitude as needed
			    print(altitude,"altitude")
			    '''
			    if same_alt_flag:
				    thread = threading.Thread(target=arm_and_takeoff, args=(vehicle, same_height))
			    else:
				    thread = threading.Thread(target=arm_and_takeoff, args=(vehicle, different_height[i]))
		...
		if(data==b"search") or (search_flag):
			...
			utils.robot.DEFAULT_SIZE= 0.4
			"""
			dce_search_file=file_name+"_search"
			if os.path.isfile(dce_search_file):
			    selected_file = dce_search_file
			else:
			    # If the DCE search file is not available, use the DCE file
