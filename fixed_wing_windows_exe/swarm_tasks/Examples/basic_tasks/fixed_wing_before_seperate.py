...
swarm_tasks.utils.robot.DEFAULT_NEIGHBOURHOOD_VAL = 40
swarm_tasks.utils.robot.DEFAULT_SIZE = 0.4
swarm_tasks.utils.robot.MAX_SPEED = 2
...
def start_search_mission(decoded_index):
    ...
    curve = BezierCurveMultiple(
        ...
        min_turn_radius=_fleet_min_turn_radius(selected_indexes),
    )
    curve.GridFormation()
...
def start_split_mission(decoded_index):
    ...
    if decoded_index.startswith("specificsplit"):
        ...
        split = SpecificSplitMission(
            ...
            min_turn_radius=_fleet_min_turn_radius(
                selected_swarm_indexes(selected_uav_ids)
            ),
        )
...
def apply_different_heights(decoded_index):
    ...
    search/split/specificsplit loop (tracked in foreground_mission_indexes) --
    ...
    busy_indexes |= set(foreground_mission_indexes)
...
# Pace each virtual leader in metres/second using its corresponding UAV's
# measured ground speed.  The allowable leader separation and curve target
# distance both scale with the configured loiter radius, so legitimate wide
# turns do not look like a tracking failure.
SIM_METRES_PER_UNIT = 2.0
UAV_DEFAULT_GROUND_SPEED_MPS = 20.0  # used before groundspeed telemetry exists
UAV_MIN_VALID_GROUND_SPEED_MPS = 5.0
UAV_DEFAULT_LOITER_RADIUS_M = 200.0
UAV_LEAD_MARGIN_M = 100.0
UAV_LEADER_SPEED_SCALE_MIN = 0.70
UAV_LEADER_SPEED_SCALE_MAX = 1.25
UAV_CURVE_CLOSE_SPEEDUP_DISTANCE_M = 100.0
UAV_CURVE_LOOKAHEAD_FACTOR = 0.80
UAV_CURVE_LOOKAHEAD_MIN_M = 65.0
UAV_CURVE_LOOKAHEAD_MAX_M = 280.0
_last_virtual_pacing_tick = {}
...
# read per control tick by _uav_loiter_radius_m) plus this margin, so it tracks
# any in-flight loiter-radius change instead of being pinned to one number.
UAV_FINAL_WAYPOINT_RADIUS_MARGIN_M = 150.0  # real metres, added to loiter radius
UAV_CURVE_POINT_SWITCH_RADIUS = 5.0  # sim units, bot-only progression
...
# WP_LOITER_RAD per real UAV, keyed by sys_id (pos_array value, not bot
# index -- survives add/remove compaction). Populated only by
# refresh_uav_loiter_radii(), which is called once per dispatched command
# (_reset_mission_state) and once at simulation init -- never from a control
# loop. _uav_loiter_radius_m() below is a pure dict lookup after that.
_loiter_radius_cache = {}


def refresh_uav_loiter_radii():
    """Fetch WP_LOITER_RAD from every connected vehicle once and cache it, so
    the per-tick control paths (_virtual_pacing_step, uav_curve_lookahead_m,
    waypoint_flyby_radius, the [bot-sync] print) never touch dronekit
    parameters again until the next command / topology change."""
    if not master_flag:
        _loiter_radius_cache.clear()
        return
    for idx, sys_id in enumerate(pos_array):
        if idx >= len(vehicles):
            continue
        try:
            radius = abs(float(vehicles[idx].parameters["WP_LOITER_RAD"]))
            if radius > 0:
                _loiter_radius_cache[int(sys_id)] = radius
        except Exception as e:
            print("[loiter-radius] fetch failed for UAV", sys_id, e)
    print("[loiter-radius] cached", _loiter_radius_cache)


def _uav_loiter_radius_m(i):
    """Return the cached loiter radius for bot index i (see
    refresh_uav_loiter_radii). Falls back to the default until the first
    refresh populates the cache, or if this UAV's fetch failed."""
    if i < len(pos_array):
        cached = _loiter_radius_cache.get(int(pos_array[i]))
        if cached:
            return cached
    return UAV_DEFAULT_LOITER_RADIUS_M


def _fleet_min_turn_radius(indexes=None):
    """The tightest turn the whole selected group can still fly = the LARGEST
    WP_LOITER_RAD among them. Passed to the Bezier generators as
    min_turn_radius so the planned turn-arounds are trackable by every UAV in
    the group instead of being shaped to the ~91 m bank-angle default. Uses
    the per-command loiter cache (refresh_uav_loiter_radii), never live
    param reads."""
    if indexes:
        idxs = [i for i in indexes if 0 <= i < len(pos_array)]
    else:
        idxs = range(len(pos_array))
    radii = [_uav_loiter_radius_m(i) for i in idxs]
    return max(radii) if radii else UAV_DEFAULT_LOITER_RADIUS_M


def _uav_ground_speed_mps(i):
    if master_flag and i < len(vehicles):
        try:
            speed = float(vehicles[i].groundspeed)
            if speed >= UAV_MIN_VALID_GROUND_SPEED_MPS:
                return speed
        except (TypeError, ValueError, AttributeError):
            pass
    return UAV_DEFAULT_GROUND_SPEED_MPS


def uav_curve_lookahead_m(i):
    """Scale the forward guidance distance to this UAV's turn radius."""
    return max(
        UAV_CURVE_LOOKAHEAD_MIN_M,
        min(
            UAV_CURVE_LOOKAHEAD_FACTOR * _uav_loiter_radius_m(i),
            UAV_CURVE_LOOKAHEAD_MAX_M,
        ),
    )


def _virtual_pacing_step(i, b, cmd_speed, separation_m, is_curve=False):
    """Match virtual motion to live ground speed with bounded lead feedback."""
    now = time.monotonic()
    nominal_tick_s = sleep_times.get(len(pos_array), 0.1)
    elapsed_s = now - _last_virtual_pacing_tick.get(i, now - nominal_tick_s)
    _last_virtual_pacing_tick[i] = now
    elapsed_s = max(0.05, min(elapsed_s, 0.5))

    loiter_radius_m = _uav_loiter_radius_m(i)
    desired_lead_m = loiter_radius_m + uav_curve_lookahead_m(i) + UAV_LEAD_MARGIN_M
    lead_scale = 1.0
    if math.isfinite(separation_m) and desired_lead_m > 0:
        if separation_m > desired_lead_m:
            # The bot is genuinely too far ahead, so let the aircraft close
            # the gap without ever freezing its forward target.
            lead_error = (desired_lead_m - separation_m) / desired_lead_m
            lead_scale = max(UAV_LEADER_SPEED_SCALE_MIN, 1.0 + 0.25 * lead_error)
        elif is_curve and separation_m < UAV_CURVE_CLOSE_SPEEDUP_DISTANCE_M:
            # On a curve, build only a small forward lead once the aircraft
            # is close.  This issues the next target continuously without
            # allowing the virtual bot to consume curve samples far ahead.
            lead_scale = UAV_LEADER_SPEED_SCALE_MAX

    effective_bot_speed = max(0.05, min(cmd_speed, b.max_speed))
    desired_virtual_m = _uav_ground_speed_mps(i) * elapsed_s * lead_scale
    return desired_virtual_m / (SIM_METRES_PER_UNIT * effective_bot_speed)


def advance_bot_with_uav_pacing(i, b, goal, label="", is_curve=False):
    """Advance the virtual leader at its UAV's measured ground speed."""
    ...
    step_size = _virtual_pacing_step(i, b, cmd.speed, dis, is_curve=is_curve)
    ...
    if BOT_SYNC_DEBUG:
        ...
        print(
            ...
            f"dis={dis:.1f}m gs={_uav_ground_speed_mps(i):.1f}m/s "
            f"loiter={_uav_loiter_radius_m(i):.0f}m curve={is_curve} "
...
def waypoint_flyby_radius(i, goal, next_goal=None, is_final=False):
    ...
    if is_final:
        radius = _uav_loiter_radius_m(i) + UAV_FINAL_WAYPOINT_RADIUS_MARGIN_M
    else:
        radius = SEARCH_UAV_FLYBY_RADIUS_M
...
def _lookahead_along_points(curve_points, bot_position, lookahead_m):
    """Project bot_position onto the polyline curve_points, then walk
    the supplied per-UAV look-ahead distance forward from that projection.
    Shared by both the
...
def curve_lookahead_goal(
    i, csv_path, line_index, num_lines, current_goal, bot_position
):
    ...
    polyline every control cycle, then walk this UAV's radius-scaled
    look-ahead distance forward.
    ...
    return _lookahead_along_points(curve_points, bot_position, uav_curve_lookahead_m(i))


def line_lookahead_goal(i, current_goal, next_goal, bot_position):
    ...
    if next_goal is None:
        return None
    return _lookahead_along_points(
        [current_goal, next_goal], bot_position, uav_curve_lookahead_m(i)
    )
...
def handle_concurrent_remove_command(command_data):
    ...
    try:
        ...
        ok = remove_uav_from_swarm(remove_bot_num)
        if ok:
            refresh_uav_loiter_radii()
        return ok
    except Exception as e:
...
def vehicle_connection():
    ...
    try:
        vehicle9 = connect(
            "udpin:{}:14559".format(ip),
            baud=115200,
            heartbeat_timeout=heartbeat_ip_timeout[8],
        )
        print("Drone9")
        num_bots += 1
    ...
    try:
        vehicle10 = connect(
            "udpin:{}:14560".format(ip),
            baud=115200,
            heartbeat_timeout=heartbeat_ip_timeout[9],
        )
        print("Drone10")
...
def _reset_mission_state():
    ...
    foreground_mission_indexes.clear()
    # Refresh the WP_LOITER_RAD cache once here, per dispatched command, so the
    # per-tick control loops only ever do a dict lookup.
    refresh_uav_loiter_radii()
    # Stale look-ahead markers from the previous mission -- the new mission's
    # first drive tick repopulates this per bot.
    uav_lookahead_points.clear()
...
# path that computes a vehicle target (_drive_vehicle_towards for goal/mission,
# and the search/split foreground loops); read by each foreground loop's GUI
# block and drawn by Gui.show_lookahead so the guidance lead is visible.
...
def _drive_goal_task(i, b, task, completed):
    ...
    if reached_goal:
        ...
        if goal_index >= len(goals):
            ...
            # A fixed-wing UAV cannot safely wait at a common centre point.
            # Enter its own preassigned ring slot immediately on arrival;
            # the slot offset keeps simultaneous arrivals separated.
...
def _drive_mission_task(i, b, task, completed):
    ...
    if is_curve:
        curve_guidance = curve_lookahead_goal(
            i, task["csv_path"], line_index, num_lines, goal_position, (b.x, b.y)
        )
    else:
        # Straight leg: same forward-lead treatment as a curve leg -- otherwise
        # the real UAV is aimed directly at the bot's own crawling position
        # with no line direction to track, and drifts off this leg.
        curve_guidance = line_lookahead_goal(i, goal_position, next_goal, (b.x, b.y))
    ...
    _dis, step_size = advance_bot_with_uav_pacing(
        i, b, goal_position, label="mission", is_curve=is_curve
    )
...
def _drive_vehicle_towards(i, b, guidance_position=None):
    if master_flag and i < len(vehicles):
        ...
        uav_lookahead_points[i] = position
...
while 1:
    ...
    try:
            ...
            try:
                ...
                if master_flag:
                    ...
                    while True:
                        ...
                        gui.show_lookahead(
                            [uav_lookahead_points.get(k) for k in range(len(s.swarm))],
                            gps_pts,
                        )
                        gui.update()
        ...
        if (data.startswith(b"search")) or (search_flag):
            ...
            my_seq = _last_seq
            diverted_indexes = set()
            # Tell apply_different_heights this loop owns these bots' movement, so
            # a concurrent 'different' only re-targets their altitude (picked up
            # by the simple_goto below next tick) instead of spawning an altitude
            # task that would divert them out of the search for good.
            foreground_mission_indexes = set(selected_indexes)
            origin = read_origin(rectangles_path)
            while 1:
                if num_bots == 10:
                    time.sleep(0.1)
                ...
                selected_index_set = set(selected_indexes)
                # Keep the "this loop owns these bots" set aligned after any
                # add/remove.
                foreground_mission_indexes = set(selected_indexes)
                ...
                for i, b in enumerate(s.swarm):
                    ...
                    _dis, step_size = advance_bot_with_uav_pacing(
                        i, b, goal, label="search", is_curve=is_curve
                    )
                    current_position = [b.x, b.y]
                    dx = abs(goal[0] - current_position[0])
                    dy = abs(goal[1] - current_position[1])
                    ...
                    if is_curve:
                        ...
                        curve_guidance = curve_lookahead_goal(
                            i, curve_csv, curve_index, curve_num_lines, goal, (b.x, b.y)
                        )
                        guidance_position = curve_guidance_position(b, curve_guidance)
                        ...
                        # Straight leg: same forward-lead treatment as a curve leg,
                        # using [goal, next_goal] as a 2-point polyline -- otherwise
                        # the real UAV is aimed directly at the bot's own crawling
                        # position and has no line direction to track, so it drifts
                        # onto its own path instead of following this leg.
                        line_guidance = line_lookahead_goal(
                            i, goal, next_goal, (b.x, b.y)
                        )
                        guidance_position = curve_guidance_position(b, line_guidance)
                    # Bot motion remains on the current CSV sample; only the fixed-wing
                    # target is moved ahead continuously through the leg.
                    position_for_uav = (
                        guidance_position
                    ...
                    uav_lookahead_points[i] = position_for_uav
        ...
        if (data.startswith(b"split")) or (data.startswith(b"specificsplit")):
            ...
            current_goals = [None] * len(pos_array)
            diverted_indexes = set()
            # Tell apply_different_heights this loop owns these bots' movement
            # (covers both plain 'split' and 'specificsplit', which share this
            # loop), so a concurrent 'different' only re-targets their altitude --
            # picked up by the simple_goto below next tick -- instead of spawning
            # an altitude task that would divert them out of the split for good.
            foreground_mission_indexes = set(selected_indexes)
            while 1:
                time.sleep(sleep_times.get(num_bots))
                check_for_new_command(my_seq)
                ...
                for i, b in enumerate(s.swarm):
                    ...
                    if reached_split_waypoint:
                        ...
                        if (
                            ...
                            print("grid_path_array", grid_path_array)
                    _dis, step_size = advance_bot_with_uav_pacing(
                        i, b, goal, label="split", is_curve=is_curve
                    )
                    guidance_position = None
                    if is_curve:
                        curve_index = (
                        ...
                        # Straight leg: same forward-lead treatment as a curve leg
                        # -- otherwise the real UAV is aimed directly at the bot's
                        # own crawling position with no line direction to track.
                        line_guidance = line_lookahead_goal(
                            i, goal, next_goal, (b.x, b.y)
                        )
                        guidance_position = curve_guidance_position(b, line_guidance)
