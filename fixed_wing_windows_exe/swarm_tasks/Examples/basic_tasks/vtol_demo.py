...
from goal_point_generator import (
...
def handle_concurrent_goal_command(command_data):
	...
	try:
		...
		automate, bearing_deg, safety_margin_m = parse_automate_goal_fields(msg_parts)
		if automate and goal_latlon and selected_indexes:
			index_to_center_xy, radius_m = build_automate_goal_assignment(
				goal_latlon, selected_indexes, guided_circle_radius,
				guided_circle_direction, bearing_deg, safety_margin_m,
			)
			assign_loiter_goal_tasks(index_to_center_xy, radius_m, guided_circle_direction)
		else:
...
def add_uav_to_swarm(sys_id):
	global vehicles, pos_array, s, num_bots, uav_home_pos, add_bot_flag, add_bot_array
	try:
		sys_id = int(sys_id)
		...
		rebuild_uav_home_positions()
		# Hand off to whichever search/split loop is currently running (if
		# any) so it can pad its own per-bot arrays and give this index a
		# mission task -- see add_bot_flag's declaration above.
		add_bot_flag = True
		add_bot_array.append(insert_index)
		uav_registry[sys_id] = {
			"sys_id": sys_id,
			"active": True,
			"index": insert_index,
...
def sync_mission_arrays_for_removed_bot(m, arrays):
	"""Pop mission-local per-bot index m from every array in `arrays`, so
	they stay aligned with pos_array/vehicles/s.swarm after
	remove_uav_from_swarm has already popped that index there. Used by the
	search/split loops draining remove_bot_flag/remove_bot_array -- keeps
	num_lines/current_goals/planned_paths_by_bot/removed_grid_path_array/etc
	from silently going stale (and misaligned with grid_path_array, which is
	popped separately since its old value also feeds the redistribution
	bookkeeping) the moment a UAV is removed mid-mission."""
	for arr in arrays:
		if 0 <= m < len(arr):
			arr.pop(m)


def sync_mission_arrays_for_added_bot(target_length, zero_default_arrays, none_default_arrays):
	"""Right-pad mission-local per-bot arrays up to target_length (==
	len(pos_array)) after add_uav_to_swarm has already appended a new slot
	there. Used by the search/split loops draining add_bot_flag/add_bot_array
	-- without this, a UAV re-added mid-mission leaves these arrays shorter
	than s.swarm/pos_array, so the new index either raises IndexError the
	first time a per-bot array is read for it, or (where a length is
	compared against len(pos_array) instead of indexed directly) silently
	disables a check for every bot until the arrays happen to line up
	again."""
	for arr in zero_default_arrays:
		while len(arr) < target_length:
			arr.append(0)
	for arr in none_default_arrays:
		while len(arr) < target_length:
			arr.append(None)
...
remove_bot_index=[]
remove_bot_array=[]
# add_bot_flag/add_bot_array mirror remove_bot_flag/remove_bot_array: set by
# add_uav_to_swarm() when a UAV is appended back onto pos_array/vehicles/s.swarm
# mid-mission, drained by the search/split loops below to pad their own
# per-bot arrays (num_lines, current_goals, planned_paths_by_bot, ...) back
# into alignment and give the new index a mission task, the same way a
# removal is drained via remove_bot_flag.
add_bot_flag=False
add_bot_array=[]
...
def assign_loiter_goal_tasks(index_to_center_xy, guided_circle_radius, guided_circle_direction):
	"""Automate-Goals variant of assign_goal_tasks: instead of every selected
	bot sharing one goals list, each bot index gets its OWN single loiter
	centre (already converted to the local sim frame). _drive_goal_task then
	flies each UAV straight to its centre and _start_guided_circle_task turns
	that into a per-UAV loiter circle -- so N UAVs end up on N non-overlapping
	circles strung along the operator's chosen bearing.

	Unlike assign_goal_tasks this passes the real loiter radius through
	(assign_goal_tasks hardcodes 350), because the centres were spaced
	2*radius + safety_margin apart by goal_point_generator and the circles
	have to match that radius or the "never overlap" guarantee is lost."""
	try:
		radius = int(float(guided_circle_radius))
	except (TypeError, ValueError):
		radius = 0
	with active_goal_tasks_lock:
		for bot_index, center_xy in index_to_center_xy.items():
			active_goal_tasks[bot_index] = {
				"type": "goal",
				"goals": [tuple(center_xy)],
				"goal_index": 0,
				"guided_circle_radius": radius if radius > 0 else 350,
				"guided_circle_direction": guided_circle_direction,
			}
	print("[loiter-goal-task] assigned", index_to_center_xy,
	      "circle_radius", radius, "circle_direction", guided_circle_direction)


def build_automate_goal_assignment(goal_latlon, selected_indexes, guided_circle_radius,
                                   guided_circle_direction,
                                   bearing_deg=DEFAULT_BEARING_DEG,
                                   safety_margin_m=DEFAULT_SAFETY_MARGIN_M):
	"""Fan the ONE operator-dropped point (goal_latlon[0]) into one loiter
	centre per selected UAV, marched along bearing_deg and spaced
	2*radius + safety_margin apart, then convert each centre to the /2-scaled
	local sim frame the goal tasks use. Returns (index_to_center_xy, radius_m)
	ready for assign_loiter_goal_tasks. Pure apart from the origin/endDistance
	globals geoToCart already needs."""
	base_lat, base_lon = float(goal_latlon[0][0]), float(goal_latlon[0][1])
	try:
		radius_m = int(float(guided_circle_radius))
	except (TypeError, ValueError):
		radius_m = 0
	if radius_m <= 0:
		# Need a real circle size to space the centres by 2r + margin.
		radius_m = 300
	try:
	...
	centers = loiter_goal_latlon_list(
		base_lat, base_lon, len(selected_indexes), radius_m,
		bearing_deg, safety_margin_m,
	)
	index_to_center_xy = {}
	for bot_index, (lat, lon) in zip(selected_indexes, centers):
		x, y = locatePosition.geoToCart(origin, endDistance, [lat, lon])
		index_to_center_xy[bot_index] = (x / 2, y / 2)
	return index_to_center_xy, radius_m


def parse_automate_goal_fields(msg_parts):
	"""Optional trailing wire fields appended after the ids JSON (msg_parts[4])
	by an Automate-Goals-aware GCS/xag-server:
	  [5] "1"/"0" automate flag   [6] bearing deg   [7] safety margin m
	Missing => automate off, generator defaults. Older senders (exactly 5
	fields) are unaffected."""
	automate = len(msg_parts) > 5 and str(msg_parts[5]).strip() == "1"
	bearing_deg = msg_parts[6] if len(msg_parts) > 6 else DEFAULT_BEARING_DEG
	safety_margin_m = msg_parts[7] if len(msg_parts) > 7 else DEFAULT_SAFETY_MARGIN_M
	return automate, bearing_deg, safety_margin_m
...
while(1):
	...
	try:            
		...
		if data.startswith(b'goal'):				
				...
				try:		
					...
					if automate and goal_latlon and selected_indexes:
						...
						print('[goal] automate -> per-UAV loiter centres', index_to_center_xy)
						assign_loiter_goal_tasks(index_to_center_xy, radius_m, guided_circle_direction)
					else:
						assign_goal_tasks(selected_indexes, goal_xy, guided_circle_radius, guided_circle_direction)
		...
		if(data.startswith(b"navigate")) or (start_flag):
			...
			for b in s.swarm:
				...
				while 1:
					...
					time.sleep(sleep_times.get(num_bots))
					check_for_new_command(my_seq)
					# Sized off len(s.swarm), not the outer num_bots -- num_bots
					# is only refreshed once per dispatched command (see the
					# very top of the outer while(1) loop), so a UAV added or
					# removed mid-navigate via check_for_new_command above
					# leaves it stale for the rest of this mission. Indexing
					# bot_array[i] below with i up to len(s.swarm)-1 against a
					# bot_array still sized off the pre-topology-change count
					# raises IndexError the moment a newly added bot reaches
					# its goal, which (per the broad except around the whole
					# dispatch loop) aborts navigate for every bot, not just
					# the new one.
					bot_array=[0]*len(s.swarm)
					for i,b in enumerate(s.swarm):
						if i in active_goal_tasks:
		...
		if(data.startswith(b"search")) or (search_flag):
			...
			diverted_indexes=set()
			origin = read_origin(rectangles_path)
			# Drop any remove/add hand-off left un-drained by a mission that
			# aborted before reaching its own remove_bot_flag/add_bot_flag
			# check below -- otherwise this fresh search would try to apply
			# stale indexes captured against a pos_array that no longer
			...
			while 1:
				...
				if(remove_bot_flag):
					...
					for m in removed_indexes:
						...
						# Every other per-bot array below also needs to lose
						# this index -- left alone, they silently drift out
						# of alignment with grid_path_array/pos_array from
						# here on: a surviving bot that shifts from index
						# m+1 down to m starts reading num_lines[m] (still
						# the just-removed bot's line count) against its own
						# (correctly shifted) grid_path_array[m], comparing
						# two different bots' progress against each other.
						sync_mission_arrays_for_removed_bot(m, (
							num_lines, current_goals, planned_paths_by_bot,
							removed_grid_path_array, removed_grid_path_array_start_val,
							checkall_removed_grid_path_array_start_val, removed_grid_filename,
						))
					...
					remove_bot_array=[]
					remove_bot_flag=False
					print("[mission-topology] synchronized after remove; search continues for", selected_indexes)

				if(add_bot_flag):
					print("add_bot_flag,add_bot_array", add_bot_flag, add_bot_array)
					added_indexes = sorted(set(idx for idx in add_bot_array if 0 <= idx < len(pos_array)))
					# Pad every per-bot array search touches back out to
					# len(pos_array) -- without this the newly appended
					# index either raises IndexError the first time one of
					# these is read for it, or (for the length-vs-len(pos_array)
					# comparisons, e.g. checkall_removed_grid_path_array_start_val
					# below) silently disables that check for every bot
					# until a subsequent removal happens to make the
					# lengths match again.
					sync_mission_arrays_for_added_bot(
						len(pos_array),
						zero_default_arrays=(
							all_uav_csv_grid_array, grid_path_array, num_lines,
							removed_grid_path_array, removed_grid_path_array_start_val,
							checkall_removed_grid_path_array_start_val, removed_grid_filename,
						),
						none_default_arrays=(current_goals, planned_paths_by_bot),
					)
					# A newly (re-)added UAV has no grid of its own here --
					# generating one would mean running the search's own
					# grid-formation math for a single extra bot mid-flight,
					# which is out of scope for this fix. Instead give it a
					# real, already-generated route (one of this mission's
					# own CSV files) via the same background mission-task
					# runner that already drives a concurrently-redirected
					# bot (see start_search_mission/assign_mission_tasks) --
					# that keeps it out of the legacy per-line loop below
					# entirely (it's skipped once it shows up in
					# active_goal_tasks) so it can never desync the indexes
					# selected_indexes still drives for the surviving bots.
					fallback_csv = csv_file_paths[0] if csv_file_paths else None
					for new_index in added_indexes:
						if fallback_csv:
							assign_mission_tasks({new_index: fallback_csv}, "search")
							print("[add] UAV", pos_array[new_index], "index", new_index, "joined search with a fallback route")
						else:
							print("[add] UAV", pos_array[new_index], "index", new_index, "added but no route available yet; holding")
					add_bot_array=[]
					add_bot_flag=False
					print("[mission-topology] synchronized after add; search continues for", selected_indexes)
				...
				for i,b in enumerate(s.swarm):
					...
					if(len(checkall_removed_grid_path_array_start_val)==len(pos_array)):
					    ...
					    pass
					if all(grid_path_array[x] >= int(num_lines[x]) for x in selected_indexes) and removed_grid_path_length!=[] and not removed_grid_path_array_flag:
					...
					if all(grid_path_array[x] >= int(num_lines[x]) for x in selected_indexes) and not removed_grid_path_length!=[]:
						landing_flag=True
			...
			while 1:
				...
				if(remove_bot_flag):
					...
					for m in removed_indexes:
						...
						if 0 <= m < len(grid_path_array):
							removed_grid_path_length.append(grid_path_array.pop(m))
						# See sync_mission_arrays_for_removed_bot's docstring:
						# without this, a surviving bot that shifts down into
						# index m reads a stale num_lines[m]/current_goals[m]/etc
						# left over from whichever bot used to be at m.
						sync_mission_arrays_for_removed_bot(m, (
							num_lines, current_goals, planned_paths_by_bot,
							removed_grid_path_array, removed_grid_path_array_start_val,
							checkall_removed_grid_path_array_start_val, removed_grid_filename,
					...
					remove_bot_array=[]
					remove_bot_flag=False
					print("[mission-topology] synchronized after remove; split continues for", selected_indexes)

				if(add_bot_flag):
					print("add_bot_flag,add_bot_array", add_bot_flag, add_bot_array)
					added_indexes = sorted(set(idx for idx in add_bot_array if 0 <= idx < len(pos_array)))
					# See sync_mission_arrays_for_added_bot's docstring --
					# same padding split needs for the same reason search does.
					sync_mission_arrays_for_added_bot(
						len(pos_array),
						zero_default_arrays=(
							all_uav_csv_grid_array, grid_path_array, num_lines,
					...
					# Same reasoning as the search loop's add_bot_flag block:
					# reuse one of this split's own already-generated routes
					# rather than computing a new group split for one extra
					# bot mid-flight, and route it through the background
					# mission-task runner so it never touches selected_indexes.
					fallback_csv = csv_file_paths[0] if csv_file_paths else None
					for new_index in added_indexes:
						if fallback_csv:
							assign_mission_tasks({new_index: fallback_csv}, "split")
							print("[add] UAV", pos_array[new_index], "index", new_index, "joined split with a fallback route")
						else:
							print("[add] UAV", pos_array[new_index], "index", new_index, "added but no route available yet; holding")
					add_bot_array=[]
					add_bot_flag=False
					print("[mission-topology] synchronized after add; split continues for", selected_indexes)
