...
# Bind the socket to the port
graph_server_address = ('192.168.50.214', 12009)  #receive from .....rx.py
...
uav4_server_address = ('192.168.50.214', 12002)
#uav5 = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
#uav5_server_address = ('192.168.50.214', 12002)
...
try:
	...
	graph_socket.sendto(str("Drone 2 master received").encode(), graph_server_address)	
except:
	pass		
while True:
	try:
		...
		print("decoded_index",decoded_index)
		graph_socket.sendto(str("Drone 2"+decoded_index).encode(), graph_server_address)
...
if(file_name=="Medur_"):
	...
	start_multiple_goals=[(4225.5128163765585,2986.100255505548),(3979.60523642999,2643.602967905786),(3587.5802265870293,2734.1606300801363),(3819.5409708283164,3124.2560304971357)]
	multiple_goals=start_multiple_goals
...
# Bind the socket to the port
server_address = ('192.168.154.74', 12005)  #receive from .....rx.py

# Bind the socket to the port
remove_bot_server_address = ('192.168.154.74', 12001)  #receive from .....rx.py
...
vehicles=[]
pos_array=[]
heartbeat_ip=["192.168.6.151","192.168.6.152","192.168.6.153","192.168.50.214","192.168.6.155","192.168.6.156","192.168.6.157","192.168.6.158","192.168.6.159","192.168.6.160"]
heartbeat_ip_timeout=[0]*10
heartbeat_ip_timeout=[30,3,3,3,3,3,3,1,1,1]
goal_path_csv_array=[]
...
def vehicle_collision_moniter_receive():	
        global index
        global vehicles
        global slave_heal_ip,master_flag,master_num,pos_array,home_pos,uav_home_pos,skip_wp_flag,next_wp,clockwise_flag,direction_flag
        global target_flag,target_uav,target_array,target_array_xy
        while 1:
        	index, address = sock3.recvfrom(1024)
        	print ("msg!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!", index)   
        	index1=index 
        	decoded_index=index1.decode('utf-8')
        	print("decoded_index",decoded_index)           
        	if decoded_index.startswith("master"):
        		...
        		m,master_num=decoded_index.split("-")
        		print("m,master_num",m,master_num)
        		msg='Drone 2 master_num '+str(master_num)+' data received'
        		for l in range(2):
        	...
        	if(decoded_index.startswith("direction")):
        		d,clock=decoded_index.split(',')
        		direction_flag=True
        		if(int(clock)==0):
        			clockwise_flag=True
        		if(int(clock)==1):
        		...
        		print("clockwise_flag",clockwise_flag)
        		
        	if(decoded_index.startswith("target")):
        		d,uav_num,t_lat,t_lon=decoded_index.split(',')
        		print("d,uav_num,t_lat,t_lon",d,uav_num,t_lat,t_lon)
        		target_flag=True
        		target_uav=int(uav_num)
        		target_array[1]=(float(t_lat),float(t_lon))
        		t_11 , t_12 = locatePosition.destination_location(float(t_lat),float(t_lon),300,270)
        		target_array[0]=(t_11 , t_12)
        		t_21 , t_22 = locatePosition.destination_location(float(t_lat),float(t_lon),300,90)        		
        		target_array[2]=(t_21 , t_22)
        		print('target_array',target_array)
        		for i,z in enumerate(target_array):
        			x,y = locatePosition.geoToCart (origin, endDistance, z)
        			target_array_xy[i]=(x/2,y/2)
        		print("target_array_xy",target_array_xy,target_flag)	
"""
...
def vehicle_connection():	
	...
	try:	
		vehicle1= connect('udpin:192.168.50.214:14551',baud=115200, heartbeat_timeout=heartbeat_ip_timeout[0])
	...
	try:		
		vehicle2= connect('udpin:192.168.50.214:14552',baud=115200,heartbeat_timeout=heartbeat_ip_timeout[1])
	...
	try:
		vehicle3= connect('udpin:192.168.50.214:14553',baud=115200,heartbeat_timeout=heartbeat_ip_timeout[2])
	...
	try:		
		vehicle4= connect('udpin:192.168.50.214:14554',baud=115200,heartbeat_timeout=heartbeat_ip_timeout[3])
	...
	try:		
		vehicle5= connect('udpin:192.168.50.214:14555',baud=115200,heartbeat_timeout=heartbeat_ip_timeout[4])
	...
	try:
		vehicle6= connect('udpin:192.168.50.214:14556',baud=115200,heartbeat_timeout=heartbeat_ip_timeout[5])
	...
	try:
		vehicle7= connect('udpin:192.168.50.214:14557',baud=115200,heartbeat_timeout=heartbeat_ip_timeout[6])
	...
	try:
		vehicle8= connect('udpin:192.168.50.214:14558',baud=115200,heartbeat_timeout=heartbeat_ip_timeout[7])
	...
	try:	
		vehicle9= connect('udpin:192.168.50.214:14559',baud=115200,heartbeat_timeout=heartbeat_ip_timeout[8])
	...
	try:	
		vehicle10= connect('udpin:192.168.50.214:14560',baud=115200,heartbeat_timeout=heartbeat_ip_timeout[9])
...
endDistance=20000
same_height=100
different_height=[100,120,140,160,180,200,220,240,260,280]
...
slave_heal_ip=["192.168.50.214"]*num_bots
...
server_address1 = ('192.168.154.74', 12010)
slave_heal_ip=["192.168.50.214"]*num_bots
...
while True:
	if(uav_home_pos!=[]):
		...
		for l in range(3):
			graph_socket.sendto(str("Drone 2 CONNECTED").encode(), graph_server_address)	
		break
	else:
		pass
#gui = viz.Gui(s)
...
while(1):
	...
	try:            
		...
		if(data.startswith(b"start")) or (circle_formation_flag) :
			...
			center_lat , center_lon = locatePosition.destination_location(float(base_lat),float(base_lon),300,0)
			...
			multiple_goals_latlon = generate_points(center_lat, center_lon, 8, 300)
					...
					for i,b in enumerate(s.swarm):
							...
							if(dx<=100 and dy<=100):
								...
								if(dx<=100 and dy<=100):
									if(direction_flag):
										...
										ind[i]+=1
									print("inddddddd",ind)
									if(abs(ind[i])==len(multiple_goals)):
										ind[i]=0
										print("ind####",ind)
								if master_flag:
									current_position = [b.x*2,b.y*2]
						...
						if(circle_formation_table[i]==1):	
							...
							if(dx<=dx_limit and dy<=dy_limit):
								print("clockwise_flag,direction_flag",clockwise_flag,direction_flag)
								if(direction_flag):
									if(clockwise_flag):
										ind[i]+=1
		...
		if(data==b"navigate") or (start_flag):			
			...
			for b in s.swarm:				
				multiple_goals=start_multiple_goals
				if(circle_formation_goals!=[]):
					multiple_goals.append(	circle_formation_goals[0])		
		...
		if(data==b"search") or (search_flag):
			...
			print("&&&&&&&",str(num_bots)+"uav")
			subdirectory = "search_grid"+"/"+str(len(pos_array))+"uav"
			print("subdirectory",subdirectory)
			# Generate file paths
			csv_file_paths = {
			    i: os.path.join(cwd, subdirectory, f"d{i+1}.csv") for i in range(0, len(pos_array))
			}
			print("csv_file_paths",csv_file_paths)
			# Print generated file paths
			for key, path in csv_file_paths.items():
			    print(f"File path for d{key}.csv: {key},{path}")			
			while 1:	
				if(num_bots==10):
					time.sleep(0.009)
				elif(num_bots==9):
				...
				if(remove_bot_flag):
					removed_uav_grid.append(all_uav_csv_grid_array.pop(remove_bot_index))
					removed_grid_path_length.append(grid_path_array.pop(remove_bot_index))
					remove_bot_flag=False
				...
				if(search_step==1):
					for i,b in enumerate(s.swarm):
						all_uav_csv_grid_array[i]=csv_file_paths.get(i)
					print("all_uav_csv_grid_array",all_uav_csv_grid_array)
					search_step+=1
				for i,b in enumerate(s.swarm):
					if all(x >= int(num_lines) for x in grid_path_array):
						if(same_alt_flag):
							return_height_flag=False
							landing_flag=True
						else:
							...
							landing_flag=True
						if(removed_grid_path_length!=[]):
							if all(x >= int(num_lines) for x in grid_path_array) and all(y >= int(num_lines) for y in removed_grid_path_length) :
								break
						else:
							break
					
					if grid_path_array[i]>=int(num_lines):						
						continue						
					...
					if r<0.4:					
						...
						dx=abs(goal[0]-plane_points[0])
						dy=abs(goal[1]-plane_points[1])
						p = Point(plane_points[0],plane_points[1])
						r = p.distance(Point(goal[0],goal[1]))
						print("radius",r)
						if(grid_path_array[i]==1):				
							if(r<102):								
								if grid_path_array[i]>=int(num_lines):
									continue
								else:
				...
				if master_flag:								
					sent = graph_socket.sendto(str(msg).encode(), graph_server_address)				
				#print("!!!!!!!!!!!!AREA covered!!!!!!!!",percentage,elapsed_time)
		...
		if(landing_flag) or data==b"return":
			...
			for b in s.swarm:
				...
				ind=[0]*num_bots
				landing_flag=True
				print("landing_flag",landing_flag)
				while 1:		
					if(not landing_flag):
						break					
					if(step==1):
					...
					for i,b in enumerate(s.swarm):
						...
						if(target_flag):
							uav=0			
							for l in range(0,len(pos_array)):	
								if(int(target_uav)==pos_array[l]):
									uav=l
							t_goal=target_array_xy[z]
							print("t_goal",t_goal)
						if(circle_formation_table[i]==0) :
							if(target_flag):
								if(i==uav):					
									cmd =cvg.goal_area_cvg(i,b,t_goal)
									cmd+= disp_field(b,neighbourhood_radius=2000)*disp_value
									cmd.exec(b)
									dx=abs(t_goal[0]-current_position[0])
									dy=abs(t_goal[1]-current_position[1])
									print("dx,dy",dx,dy)
								else:	
								...
								if(dx<=10 and dy<=10):
									...
									plane_points=[x/2,y/2]			
									if(target_flag):
										dx=abs(t_goal[0]-plane_points[0])
										dy=abs(t_goal[1]-plane_points[1])
										print("target!!!!!!!",dx,dy)
										if(dx<=100 and dy<=100):
											z+=1
											print("z",z)
		...
		if(data==b"home") or (home_flag):
			...
			while 1:
				...
				for i,b in enumerate(s.swarm):
					...
					if master_flag:										
						current_position = [b.x*2,b.y*2]
						#print("Drone 2 position",b.x,b.y)
						lat,lon = locatePosition.cartToGeo (origin, endDistance, current_position)
						#print ("Drone 2 lat,lon", lat,lon)
						if same_alt_flag:
							point1 = LocationGlobalRelative(lat,lon,same_height)
