...
from gridplanwithbeziercurve import AutoSplitMission
...
while(1):
	...
	try:            
		...
		if(data.startswith(b"split")) or (split_flag):
			print("data",data)
			decoded_index = data.decode('utf-8')  # Assuming utf-8 encoding, adjust if needed
			msg_parts = decoded_index.split('_')
			print('msg_parts',msg_parts,len(msg_parts))
			...
			print("split",split,center_lat_lon_array,int(num_uavs),int(grid_space),int(coverage_area))
