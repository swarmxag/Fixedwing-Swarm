...
while(1):
	...
	try:            
		...
		if(data==b"start") or (start_flag):
			...
			multiple_goals=[(1295.5722521723492,4071.6967245074884),(1491.610145264652,3760.6696265588407),(1222.7428646501228,3579.209562083801),(1007.5319655105639,3871.5499091672796)]	
			for b in s.swarm:				
		...
		if(data==b"search") or (search_flag):
			...
			covered_points=0			
			csv_file_paths = {
			    0: '/home/dhaksha/Downloads/dec-3-drone-new/grid-csv/dec_3_d1.csv',
			    1: '/home/dhaksha/Downloads/dec-3-drone-new/grid-csv/dec_3_d2.csv',
			    2: '/home/dhaksha/Downloads/dec-3-drone-new/grid-csv/dec_3_d3.csv'
			    # Add more paths for other values of i if needed
			}			
			while 1:	
				if(num_bots==10):
					time.sleep(0.005)
				...
				for i,b in enumerate(s.swarm):
					...
					if grid_path_array[i]>=int(num_lines):
						"""
						if(removed_uav_grid!=[]) and (removed_grid_path_length!=[]) and grid_completed_bot[i]==-1:
							grid_completed_bot[i]=0
							print("grid_completed_bot",grid_completed_bot)
						else:	
						"""
						continue						
					'''													
					...
					'''		
					"""			
					if(grid_completed_bot[i]==0):
						goal_lat_lon = read_specific_line(removed_uav_grid[0], removed_grid_path_length[0])
						print("goal_lat_lon",goal_lat_lon)
					else:
					"""
					...
					if(dx<=5 and dy<=5):
						...
						if(grid_completed_bot[i]==0):
							print("!!!!!!!")
							if removed_grid_path_length[i]>=int(num_lines):
								continue
							else:
								removed_grid_path_length[i]+=1
						else:
						"""
						if grid_path_array[i]>=int(num_lines):
							continue
				...
				if(return_height_flag):
					...
					while True:	
						...
						if(vehicle_lost_flag):
							...
							print(x)
						for i, b in enumerate(s.swarm):							
							cmd = potf.velocity(b.get_position(), b.sim, weights=potf.field_weights, order = 2, max_dist=10)
							cmd.exec(b)
							if master_flag:
								value=[b.x*2,b.y*2]					
								lat,lon = locatePosition.cartToGeo (origin, endDistance, value)
