...
if(file_name=="dce_airport"):
	...
	multiple_goals=start_multiple_goals
	return_multiple_goals=start_multiple_goals
...
Heartbeat_ip_timeout=[30,30,3,3,3,3,3,3,3,3]
goal_path_csv_array=[]
					...
					if(step==1):											
						...
						for i,b in enumerate(s.swarm):							
							...
							if first!=0:
								...
								while True:
									if uav_bot_array[i]==1 or all_uav_reach_flag:
										break
									else:
										for vehicle in vehicles:
											lat = vehicle.location.global_relative_frame.lat
											...
											for j in range(len(vehicles)):
												#print("************")
												if(uav_bot_array[j]==1):
													count_1+=1
												if(count_1==len(vehicles)):
														count_1=0
													uav_bot_array=[0]*len(vehicles)
													all_uav_reach_flag=True
											"""
							
							if first==0 or uav_bot_array[i]==1 or all_uav_reach_flag:
							#if first==0 or all_uav_reach_flag:
								all_uav_reach_flag=False
								all_bot_reach_flag=False
								cmd =cvg.goal_area_cvg(i,b,goal,3)								
								#Execute
								cmd.exec(b)
								...
								if master_flag and (all_bot_reach_flag==True or first>=0): 
									...
									vehicles[i].simple_goto(point1)
									print("Gotooooooooo")
									uav_bot_array[i]=0*len(vehicles)
									if i==0:
									    robot1_x,robot1_y={b.x},{b.y}
									elif i==1:
									    robot2_x,robot2_y={b.x},{b.y}
