    ...
    async def socket_response(
        ...
        if msg == "goal":
            if get_origin() is None:
                ...
                # "Automate Goals" strategy: the operator drops ONE point and
                # the swarm computer fans it into one loiter centre per selected
                # UAV, marching along automateBearing, spaced so the circles
                # never overlap. bearing/margin fall back to the swarm
                # computer's own defaults (90 deg / 50 m) when the GCS omits
                # them; automate off keeps the legacy "every UAV to this point"
                # behaviour.
                automate = bool(parameters.get("automateGoals", False))
                bearing_deg = parameters.get("automateBearing", 90.0)
                safety_margin_m = parameters.get("automateSafetyMargin", 50.0)
                result = goal_socket(
        ...
        if msg == "remove_link":
            # remove_link always targets exactly one UAV (parameters["id"]),
            # never the whole swarm -- unlike search/goal above, there is no
            # "requested_ids empty" case to fall back to stop_socket() for.
            # stop_socket() broadcasts "stop" on the vehicle-collision socket
            # (port 12002), which every foreground mission loop in
            # vtol_demo.py (search/split/navigate/...) treats as "abandon the
            # whole mission" at the end of its current tick -- so calling it
            # here killed the rest of the swarm's in-progress mission a full
            # second before the actual remove,<id> command (sent via the
            # separate port-12008 socket) was even received. vtol_demo.py's
            # remove_uav_from_swarm() already handles a single-UAV remove
            # concurrently, without disturbing the rest of the swarm, so
            # nothing here needs to stop first.
            uav = int(parameters.get("id"))
            result = mavlink_remove(uav)
            result = True

        if msg == "add_link":
            # See remove_link above -- add_link is likewise always scoped to
            # one UAV, and add_uav_to_swarm() already handles it concurrently
            # without stopping the rest of the swarm's mission.
            uav = int(parameters.get("id"))
        ...
        if msg == "startsimulation":
            ...
            print("ccccccccccccccccccccc",sys.executable)
