...
share_data_server_address3 = (ip, 12008)
...
share_data_server_address5 = (ip, 12008)
# udp_socket6 = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
# server_address6 = ('192.168.6.156', 12008)
...
share_data_server_address10 = (ip, 12008)
...
file_server_address10 = (ip, 12003)
...
mavlink_server_address3 = (ip, 12045)
...
mavlink_server_address5 = (ip, 12045)
...
mavlink_server_address10 = (ip, 12045)
...
def goal_socket(
    ...
    for num in goal_num:
        num.reverse()
    # Wire format is "_"-split positional. Fields 0-4 (goal/points/direction/
    # radius/ids) are the original protocol; fields 5-7 (automate flag /
    # bearing / safety margin) are appended for the "Automate Goals" strategy
    # -- when automate is on the swarm computer fans goal_num[0] into one
    # loiter centre per selected UAV along bearing_deg, spaced
    # 2*radius + safety_margin apart. Older swarm builds simply ignore the
    # extra fields; a blank/False automate keeps the legacy behaviour.
    data = str(
        "goal"
        + "_"
        + json.dumps(goal_num)
...
def bot_remove(remove_uav_num):
    ...
    data = "remove_bot" + "_" + str(remove_uav_num)
