...
# Local network IP used as the default server_address for run_server_exe()
ip = NetworkIP.get_ip()


def run_server_exe(sim_enable=False, server_address=ip):

    if getattr(sys, "frozen", False):
        base_dir = os.path.dirname(sys.executable)
    else:
        base_dir = os.path.dirname(os.path.abspath(__file__))
    swarm_exe = os.path.join(base_dir, "../xagFixedWgSwarm", "vtol_demo.exe") 
    FIXED_WING_SCRIPT = os.environ.get(
    # "FIXED_WING_SCRIPT",
    # r"D:\Kamikaze-Fuze-GUI\fixed_wing_windows_exe\swarm_tasks"
    r".\xagFixedWgSwarm\vtol_demo.exe",
)
